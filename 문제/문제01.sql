...
select last_name|| ' ' ||first_name as name, phone_number
from employees
...
select last_name|| ' ' || first_name as name, phone_number, hire_date
from employees
where last_name = 'Grant';

-- 10.employees
-- 특정 매니저(manager_id: 120)이 관리하는 직원의 이름, 급여, 연락처를 가져오시오.
select last_name|| ' ' || first_name as name, salary, phone_number
from employees
...
select last_name|| ' ' || first_name as name, phone_number, email, department_id
from employees
